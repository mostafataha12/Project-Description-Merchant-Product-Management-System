...
def debug_db():
    ...
    return render_template("debug.html", users=users, products=products)

#delet function
@app.route("/delete/<int:pro_id>", methods=["POST"], endpoint='delete')
@login_required
def delete(pro_id):
    # Find the product
    product = Product.query.filter_by(pro_id=pro_id).first()
    
    # Check if product exists and belongs to the logged-in user
    if not product:
        return render_template("error.html", message="Product not found")
    
    if product.username != session['username']:
        return render_template("error.html", message="You are not authorized to delete this product")
    
    # Save the image filename to delete
    ...
    try:
        ...
        # If file deletion fails, continue anyway
        pass
    
    # Redirect back to home page
    rows = Product.query.filter_by(username=session['username'])
    return render_template("home.html", rows=rows, message="Product deleted successfully")
